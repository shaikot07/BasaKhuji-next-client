...
const AllHousePage = ({ searchParams }: { searchParams: SearchParams }) => {
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <AllHousePageContent searchParams={searchParams} />
    </Suspense>
  );
};

const AllHousePageContent = async ({ searchParams }: { searchParams: SearchParams }) => {
  const query = await searchParams;
  const allHouse = await getAllHouse(query);
